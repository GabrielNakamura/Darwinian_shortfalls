...
l_cas <- vector(mode = "list", length = length(not_found_fb$user_spp)) 
pb = txtProgressBar(min = 0, max = length(l_cas), initial = 1, style = 3) 
for(i in 1:length(not_found_fb$user_spp)){
  # i = 1
  tryCatch(
    l_cas[[i]]<- rFishTaxa::search_cas(query = gsub(" ", "_", not_found_fb$user_spp[i]), type = "species"),
    error = function(e){
      paste("not_found", not_found_fb$user_spp[i], sep = "_")
    }
  )
...
spp_not_cas <- not_found_fb$user_spp[which(unlist(lapply(list_not_cas, function(x) length(x) >= 1)) == TRUE)]
df_validation[match(spp_not_cas, df_validation$sp.gbif), "not.cas.fb"] <- TRUE # not in cas neither fb
df_cas[match(names(which(table(df_cas$query) >= 2)), df_cas$query), ] # species with more than one register in the search
query_cas <- 
...
df_validation[, "join.fb.cass"] <- joint.cas.fb
