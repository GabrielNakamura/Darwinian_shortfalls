...
phy_fresh <- ape::read.tree(here::here("output", "phylo_freshwater.phylo"))
insertions_marine <- res_phylo_marine$Insertions_data
insertions_fresh <- read.table(here::here("output", "insertions_freshwater.txt"))
...
df_insertion_tree <- data.frame(df_insertion_marine$insertions)
rownames(df_insertion_tree) <- df_insertion_marine$s
...
names_df_order <- names(sort(table(insertions_marine$o), decreasing = TRUE)[1:5])
...
# phylogeny marine
ggsave(filename = here::here("output", "images", "phylo_all_marine.png"),
